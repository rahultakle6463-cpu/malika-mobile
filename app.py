            ...
            with st.form("tagging_form"):
                ...
                for i, item in enumerate(st.session_state.transcript_data):
                    ...
                    with col2:
                        ...
                        with col_t1:
                            try:
                                selected_tag = st.pills(f"Select Character", tag_options, default=tag_options[default_tag_idx], key=f"sel_{i}")
                                if not selected_tag:
                                    selected_tag = tag_options[default_tag_idx]
...
with tab6:
    ...
    with col2:
        ...
        with st.form("gemini_mobile_form"):
            gemini_result = st.text_area("Paste the Marathi script generated by Gemini here:", height=300)
            submitted = st.form_submit_button("✅ Save Script (Click to Show Download)")
            
        if submitted or gemini_result.strip():
            html_script = f"""
            <html>
